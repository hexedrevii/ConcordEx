...
--- @param name string Name of the ComponentClass to reject
--- @return string
...
---@return boolean success True if registered, false otherwise
---@return ComponentClass|string result The ComponentClass on success, or an error string on failure
---@return boolean? isRejected True if acceptRejected was true and the name had the Reject Prefix, false otherwise
...
function Components.get(name)
    ...
    if type(value) == "string" then error("Mismatched type.", 2) end
