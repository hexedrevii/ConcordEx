...
---@class Entity
---@field SERIALIZE_BY_DEFAULT boolean
---@field __world World
---@field __isEntity boolean
local Entity     = {
...
---@param name string The name of the ComponentClass to add an instance of
...
--- @param name string the name of the ComponentClass to remove
--- @return Entity
    ...
    for i = 1, #data do
        ...
        if (not Components.has(componentData.__name)) then
            Utils.error(2, "bad argument #1 to 'Entity:deserialize' (ComponentClass '%s' wasn't yet loaded)",
                tostring(componentData.__name))                                                                                            -- luacheck: ignore
        end
